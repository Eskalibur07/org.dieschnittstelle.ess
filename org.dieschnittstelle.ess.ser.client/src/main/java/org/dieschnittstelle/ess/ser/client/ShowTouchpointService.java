...
public class ShowTouchpointService {
	...
	public void deleteTouchpoint(AbstractTouchpoint tp) {
		...
		try {
			...
			HttpDelete req = new HttpDelete("http://localhost:8888/org.dieschnittstelle.ess.ser/api/touchpoints/"+tp.getId());

			// execute the request, which will return a Future<HttpResponse> object
			Future<HttpResponse> responseFuture = client.execute(req, null);
	...
	public AbstractTouchpoint createNewTouchpoint(AbstractTouchpoint tp) {
		...
		try {
			...
			show("bos before writeObj(): %s", bos);
			// write the object to the output stream
			oos.writeObject(tp);

			show("bos after writeObj(): %s", bos);
